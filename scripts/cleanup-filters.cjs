...
filesToFix.forEach(relPath => {
  ...
  // We need to remove the false blocks that got added
  const falseLabels = ['Title Card', 'Filters Card', 'Table Section', 'Top Controls', 'Pagination', 'Recent Purchases'];
  ...
  falseLabels.forEach(label => {
    ...
    // Replace the entire block with just the comment again
