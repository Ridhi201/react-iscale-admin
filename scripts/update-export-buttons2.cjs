...
function processDirectory(dir) {
  ...
  for (const f of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      const regex1 = /<div className="flex gap-0 border border-slate-300[^>]+>[\s\S]*?\['Copy',\s*'Excel',\s*'PDF',\s*'Print'\]\.map\([\s\S]*?<\/button>[\s\S]*?<\/div>/g;
      
      // Matches the secondary button style found in CourseWishlist etc.
      const regex2 = /<div className="flex gap-2">\s*<button[^>]*>Copy<\/button>\s*<button[^>]*>Excel<\/button>\s*<button[^>]*>PDF<\/button>\s*<button[^>]*>Print<\/button>\s*<\/div>/g;
      
      let changed = false;
      if (regex1.test(content)) {
        content = content.replace(regex1, replaceStr);
        changed = true;
      }
      if (regex2.test(content)) {
        content = content.replace(regex2, replaceStr);
        changed = true;
      }
