...
export default function NotesWishlist() {
  ...
  const handleView = async (id) => {
    try {
      const token = localStorage.getItem('token')
      const response = await axios.get(`${BASE_URL}/myadmin/user-wishlist/notes/admin/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data.status) {
  ...
  return (
    <div className="h-full animate-fade-in-up">
      ...
      <div className="bg-[#144f36] rounded-t-2xl p-5 flex justify-between items-center shadow-md relative overflow-hidden group mb-5">
        <h2 className="text-white font-bold tracking-tight text-xl relative z-10">Notes Wishlist List</h2>
      </div>
          ...
          <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
            <thead className="bg-slate-50 dark:bg-[#13111c] text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-gray-800">
              <tr>
                ...
                <th className="px-3 py-3 font-semibold text-xs border-r border-slate-200 dark:border-gray-800/50 align-top">Notes Title</th>
                ...
                data.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-xs border-r border-slate-200 dark:border-gray-800/50 text-slate-600 dark:text-slate-400">
                      {row.notes_name || row.notesTitle || row.notes_id}
                    </td>
                    <td className="px-3 py-3 text-xs border-r border-slate-200 dark:border-gray-800/50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-2xl relative shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-4">
              ...
              <div>
                <p className="font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider mb-1">Notes Title</p>
                <p className="text-gray-800 dark:text-gray-200">{selectedWishlist.notes_id?.notes_name}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider mb-1">Notes Price</p>
                <p className="text-gray-800 dark:text-gray-200">₹{selectedWishlist.notes_id?.notes_price || 0}</p>
              </div>
              <div>
