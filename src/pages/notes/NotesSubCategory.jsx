...
import { notesSubCategoryData } from '../../utils/mockData'

const TOTAL_ENTRIES = notesSubCategoryData.length
...
export default function NotesSubCategory() {
  ...
  const currentData = notesSubCategoryData.slice(startIndex, endIndex)
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col h-full">
        <div className="p-4 border-b border-slate-200 dark:border-gray-800/50 flex justify-between items-center bg-[#f6f6ff] dark:bg-[#1f1b2e]">
          <h2 className="text-xl font-medium text-indigo-900 dark:text-indigo-300 font-bold tracking-tight">Sub Category List</h2>
          <button onClick={() => navigate('/notes/sub-category/add')} className="bg-[#428bca] text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-[#3071a9] transition-colors flex items-center gap-2">
            <span>+ Add New</span>
          </button>
        </div>
            ...
            <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
              <thead className="bg-slate-50 dark:bg-[#13111c] text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-gray-800">
                <tr>
                  ...
                  <th className="px-4 py-3 font-bold border-r border-slate-200 dark:border-gray-800/50 whitespace-nowrap">Sub Category Name ( हिंदी )</th>
              ...
              <tbody>
                {currentData.map((row) => (
                  <tr key={row.id} className="border-b border-slate-200 dark:border-gray-800/50 hover:bg-slate-50 dark:bg-[#1f1b2e]/50">
                    ...
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.subCategoryName}</td>
