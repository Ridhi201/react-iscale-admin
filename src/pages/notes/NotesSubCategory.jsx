...
export default function NotesSubCategory() {
  ...
  const fetchSubCategories = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/myadmin/notes-sub-category/all?limit=1000`, {
  ...
  const handleDelete = async (id) => {
    if (!await window.customConfirm('Are you sure you want to delete this subcategory?')) return

    try {
      const token = localStorage.getItem('token')
      const response = await axios.delete(`${BASE_URL}/myadmin/notes-sub-category/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data?.status) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const token = localStorage.getItem('token')
      const response = await axios.put(`${BASE_URL}/myadmin/notes-sub-category/change-status/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data?.status) {
        fetchSubCategories()
  ...
  const filteredSubCategories = subCategories.filter(sub => 
    sub.notes_subcategory_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    sub.notes_category_id?.nc_name?.toLowerCase().includes(searchQuery.toLowerCase())
  )

  const TOTAL_ENTRIES = filteredSubCategories.length
  ...
  const currentData = filteredSubCategories.slice(startIndex, endIndex)
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md border border-slate-100 overflow-hidden flex flex-col h-full">
        <div className="p-4 flex justify-between items-center bg-[#144f36] text-white">
          <h2 className="text-xl font-bold tracking-tight text-white">Sub Category List</h2>
          <button onClick={() => navigate('/notes/sub-category/add')} className="bg-white/10 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-white/20 transition-colors flex items-center gap-2 border border-white/30">
            <span>+ Add New</span>
          </button>
        </div>
            ...
            <table className="w-full text-left text-sm text-slate-800">
              <thead className="bg-[#144f36] text-white border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold border-r border-[#0f3d2a] whitespace-nowrap">Sub Category Name</th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="8" className="text-center py-8">Loading subcategories...</td>
                  </tr>
                  ...
                  currentData.map((row, index) => (
                    <tr key={row._id} className="border-b border-slate-200 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 border-r border-slate-200 align-middle">{row.notes_subcategory_name}</td>
                      <td className="px-4 py-3 border-r border-slate-200 align-middle">
                        {row.notes_subcategory_icon ? (
                          <div className="w-10 h-10 rounded flex items-center justify-center overflow-hidden bg-slate-100">
                            <img src={getImageUrl(row.notes_subcategory_icon)} alt="icon" className="w-full h-full object-cover" onError={(e) => e.target.src = 'https://placehold.co/40x40/f1f5f9/94a3b8?text=ICN'} />
                          </div>
                        ) : '-'}
                      </td>
                      <td className="px-4 py-3 border-r border-slate-200 align-middle">
                        {row.notes_subcategory_banner ? (
                          <div className="w-16 h-10 rounded flex items-center justify-center overflow-hidden bg-slate-100">
                            <img src={getImageUrl(row.notes_subcategory_banner)} alt="banner" className="w-full h-full object-cover" onError={(e) => e.target.src = 'https://placehold.co/64x40/f1f5f9/94a3b8?text=BNR'} />
                          </div>
                        ) : '-'}
                      </td>
                      <td className="px-4 py-3 border-r border-slate-200 align-middle">
                        <div className="text-xs text-slate-600 line-clamp-3 leading-relaxed">
                          {row.notes_subcategory_description}
                        </div>
                      </td>
                      <td className="px-4 py-3 border-r border-slate-200 align-middle">
                        <button 
                          onClick={() => handleToggleStatus(row._id, row.notes_subcategory_status)}
                          className={`px-3 py-1 rounded-full text-white text-xs font-bold shadow-sm ${row.notes_subcategory_status === 'active' ? 'bg-[#144f36]' : 'bg-[#d87025]'}`}
                        >
                          {row.notes_subcategory_status === 'active' ? 'Active' : 'In-Active'}
                        </button>
                      </td>
                      <td className="px-4 py-3 align-middle">
                        <div className="flex gap-1.5">
                          <button onClick={() => navigate(`/notes/sub-category/add`, { state: { editSubCategory: row } })} className="bg-[#28a745] text-white p-1.5 rounded-full hover:bg-[#218838] transition-colors">
                            <Edit2 size={14} />
                          </button>
                  ...
                  <tr>
                    <td colSpan="8" className="text-center py-8">No subcategories found.</td>
                  </tr>
                )}
              </tbody>
            </table>
