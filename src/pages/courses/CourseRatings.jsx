...
import { courseRatingsData } from '../../utils/mockData'

const TOTAL_ENTRIES = courseRatingsData.length
...
export default function CourseRatings() {
  ...
  const currentData = courseRatingsData.slice(startIndex, endIndex)
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col h-full">
        <div className="p-4 flex justify-between items-center flex-wrap gap-4 bg-[#144f36] text-white rounded-t-2xl">
          <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">All Testimonials/Course Ratings</h2>
        </div>
            ...
            <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
              <thead className="bg-slate-50 dark:bg-[#13111c] text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-gray-800">
                <tr>
                  ...
                  <th className="px-4 py-3 font-bold border-r border-slate-200 dark:border-gray-800/50 whitespace-nowrap">Course</th>
              ...
              <tbody>
                {currentData.map((row) => (
                  <tr key={row.id} className={`border-b border-slate-200 dark:border-gray-800/50 ${row.name === '' ? 'bg-[#f6f6ff] dark:bg-[#111827] hover:bg-slate-50 dark:bg-[#1f1b2e]/50 dark:hover:bg-[#1f2937] transition-colors' : 'bg-[#fee2e2]'}`}>
                    ...
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.course}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.rating}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.review}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.addedOn}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">
                      <span className={`px-4 py-1.5 rounded text-white text-xs ${row.status === 'In-Active' ? 'bg-[#144f36]' : 'bg-[#144f36]'}`}>
                        {row.status}
                      </span>
                    </td>
