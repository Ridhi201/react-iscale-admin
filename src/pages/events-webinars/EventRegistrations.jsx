...
import { allEventRegistrationData } from '../../utils/mockData'

const TOTAL_ENTRIES = allEventRegistrationData.length
...
export default function EventRegistrations() {
  ...
  const currentData = allEventRegistrationData.slice(startIndex, endIndex)
  ...
  return (
    <div className="h-full animate-fade-in-up">
      {/* Title Card */}
        ...
        <h2 className="text-indigo-900 dark:text-indigo-300 font-bold tracking-tight text-xl font-medium">Event Enrollment List</h2>
        <button className="px-4 py-1.5 bg-[#428bca] text-white rounded-full text-sm hover:bg-[#3071a9] transition-colors flex items-center gap-1">
            ...
            <tbody>
              {currentData.map((row) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-xs border-r border-slate-200 dark:border-gray-800/50 text-slate-600 dark:text-slate-400">{row.event}</td>
                  <td className="px-3 py-3 text-xs border-r border-slate-200 dark:border-gray-800/50">
