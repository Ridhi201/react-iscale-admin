...
import { leadsData } from '../../utils/mockData'

const TOTAL_ENTRIES = leadsData.length
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col h-full">
        <div className="p-4 border-b border-slate-200 dark:border-gray-800/50 flex justify-between items-center bg-[#f6f6ff] dark:bg-[#1f1b2e]">
          <h2 className="text-xl font-medium text-indigo-900 dark:text-indigo-300 font-bold tracking-tight">Lead Generate List</h2>
          <button 
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className="overflow-x-auto border border-slate-200 dark:border-[#1f1b2e] flex-1">
            <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
              ...
              <tbody>
                {currentData.map((row) => (
                  <tr key={row.id} className={`border-b border-slate-200 dark:border-gray-800/50 ${row.id === 4 ? 'bg-[#fee2e2]' : 'bg-[#f6f6ff] dark:bg-[#1f1b2e] hover:bg-slate-50 dark:bg-[#1f1b2e]/50'}`}>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.sno}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.title}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                    ...
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                      <span className="bg-green-600 text-white px-4 py-1.5 rounded-full text-xs inline-block">
                        {row.status}
                      </span>
                    </td>
