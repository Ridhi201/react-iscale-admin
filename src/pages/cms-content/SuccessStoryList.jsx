...
import { successStoriesData } from '../../utils/mockData'

export default function SuccessStoryList() {
  const navigate = useNavigate()
  const [currentPage, setCurrentPage] = useState(1)
  const [entriesPerPage, setEntriesPerPage] = useState(50)
  const TOTAL_ENTRIES = 40

  const currentData = successStoriesData
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col h-full">
        <div className="p-4 border-b border-slate-200 dark:border-gray-800/50 flex justify-between items-center bg-[#f6f6ff] dark:bg-[#1f1b2e]">
          <h2 className="text-xl font-medium text-indigo-900 dark:text-indigo-300 font-bold tracking-tight">Success Story List</h2>
          <button 
        ...
        <div className="p-4 flex-1 flex flex-col">
                  ...
                  <tr key={row.id} className="border-b border-slate-200 dark:border-gray-800/50 hover:bg-slate-50 dark:bg-[#1f1b2e]/50 bg-[#f6f6ff] dark:bg-[#1f1b2e]">
                    ...
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                      <div className="w-16 h-16 mx-auto bg-slate-100 dark:bg-slate-700 flex flex-col items-center justify-center border border-slate-200 dark:border-[#1f1b2e]">
                        {row.hasImage ? (
                          <div className="w-full h-full flex items-center justify-center relative bg-gradient-to-tr from-cyan-400 to-fuchsia-400">
                             <div className="w-10 h-10 bg-[#f6f6ff] dark:bg-[#1f1b2e] rounded-full flex items-center justify-center">
                               <img src={`https://api.dicebear.com/7.x/initials/svg?seed=${row.name}`} alt={row.name} className="w-8 h-8 rounded-full" />
                             </div>
                             <div className="absolute bottom-0 text-[8px] bg-[#f6f6ff] dark:bg-[#1f1b2e] text-black font-bold px-1 w-full text-center truncate">{row.name.substring(0,6)}</div>
                          </div>
                        ) : (
                          <>
                    ...
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top w-40">{row.name}</td>
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top w-32">{row.designation}</td>
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top w-32">{row.placedAt}</td>
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top w-32">{row.package}</td>
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                      <button className="bg-slate-50 dark:bg-[#13111c] text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-gray-800 px-4 py-1.5 rounded-full text-xs hover:bg-[#152a4a] transition-colors">
                        Link
                      </button>
                    </td>
                    ...
                    <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                      <button className={`text-white px-4 py-1.5 rounded-full text-xs transition-colors ${row.status === 'Active' ? 'bg-green-600 hover:bg-green-700' : 'bg-[#6366f1] hover:bg-[#d87025]'}`}>
                        {row.status}
                      </button>
                    </td>
          ...
          <div className="mt-4 flex flex-col md:flex-row justify-between items-center text-sm text-slate-800 dark:text-slate-200">
            <div className="mb-4 md:mb-0">
              Showing 1 to 40 of {TOTAL_ENTRIES} entries
            </div>
