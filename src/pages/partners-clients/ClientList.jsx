...
export default function ClientList() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/myadmin/client/get-all-client`, {
        params,
      ...
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col flex-1 min-h-0">
        <div className="bg-[#144f36] rounded-t-2xl p-5 flex justify-between items-center shadow-md relative overflow-hidden group">
          ...
          <div className="flex items-center relative z-10">
            ...
            <h2 className="text-white font-bold tracking-wide text-2xl drop-shadow-[0_2px_4px_rgba(0,0,0,0.2)]">Client List</h2>
          </div>
            ...
            <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
              ...
              <tbody>
                {loading ? (
                  ...
                  data.map((row, index) => (
                    <tr key={row._id || index} className="border-b border-slate-200 dark:border-gray-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top">
                        {row.m_client_logo ? (
                          <div className="w-16 h-16 rounded overflow-hidden bg-slate-100 flex items-center justify-center">
                            <img src={getImageUrl(row.m_client_logo)} alt={row.m_client_name} className="w-full h-full object-cover" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
                            <div className="hidden text-xs text-slate-400">No Image</div>
                          </div>
                        ) : (
                          <div className="w-16 h-16 rounded overflow-hidden bg-slate-100 flex items-center justify-center text-xs text-slate-400">
                            No Logo
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top">
                        {row.m_client_company || '-'}
                      </td>
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top">
                        {row.m_client_order || 0}
                      </td>
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                        <span className={`text-white px-4 py-1.5 rounded-full text-xs transition-colors ${row.m_client_status === 'active' || String(row.m_client_status) === '1' ? 'bg-[#144f36]' : 'bg-red-500'}`}>
                          {row.m_client_status === 'active' || String(row.m_client_status) === '1' ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-4 py-4 align-top">
                        <div className="flex items-center gap-2">
                          <button 
                            onClick={() => navigate(`/our-clients/edit/${row._id}`, { state: { client: row } })}
