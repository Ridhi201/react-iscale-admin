...
const TOTAL_ENTRIES = teamsData.length
...
export default function TeamsList() {
  ...
  const currentData = teamsData.slice(startIndex, endIndex)
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col h-full">
        <div className="p-4 border-b border-slate-200 dark:border-gray-800/50 flex justify-between items-center bg-[#f6f6ff] dark:bg-[#1f1b2e]">
          <h2 className="text-xl font-medium text-indigo-900 dark:text-indigo-300 font-bold tracking-tight">Teams List</h2>
          <button 
        ...
        <div className="p-4 bg-[#eef2f6]/50 border-b border-slate-200 dark:border-gray-800/50 flex gap-4 items-end">
          <div>
            <label className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">Type</label>
            <select className="border border-slate-300 dark:border-gray-700 bg-[#f6f6ff] dark:bg-[#13111c] text-slate-700 dark:text-slate-300 rounded px-3 py-2 text-sm outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 w-48 bg-[#f6f6ff] dark:bg-[#1f1b2e]">
              ...
              <tbody>
                {currentData.map((row) => (
                  <tr key={row.id} className="border-b border-slate-200 dark:border-gray-800/50 hover:bg-slate-50 dark:bg-[#1f1b2e]/50">
                    ...
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.name}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.position}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.expertise}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top text-blue-600">
                      {row.experience} {row.linkedin && `| ${row.linkedin}`}
                    </td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.type}</td>
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.status}</td>
