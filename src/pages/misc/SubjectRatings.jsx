...
import { subjectRatingsData } from '../../utils/mockData'

const TOTAL_ENTRIES = subjectRatingsData.length

export default function SubjectRatings() {
  ...
  const currentData = subjectRatingsData.slice(startIndex, endIndex)
    ...
    <div className="h-full animate-fade-in-up">
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col h-full">
        <div className="p-4 border-b border-slate-200 dark:border-gray-800/50 flex justify-between items-center bg-[#f6f6ff] dark:bg-[#1f1b2e]">
          <h2 className="text-xl font-medium text-indigo-900 dark:text-indigo-300 font-bold tracking-tight">Subject Rating List</h2>
          <button onClick={() => navigate('/subject-ratings/add')} className="bg-[#428bca] text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-[#3071a9] transition-colors flex items-center gap-2">
            <span>+ Add New</span>
          </button>
        </div>
              ...
              <tbody>
                {currentData.map((row) => (
                  <tr key={row.id} className="border-b border-slate-200 dark:border-gray-800/50 hover:bg-slate-50 dark:bg-[#1f1b2e]/50">
                    ...
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">{row.subject}</td>
                    ...
                    <td className="px-4 py-3 border-r border-slate-200 dark:border-gray-800/50 align-top">
                      <span className={`px-4 py-1.5 rounded text-white text-xs ${row.status === 'In-Active' ? 'bg-[#6366f1]' : 'bg-[#428bca]'}`}>
                        {row.status}
                      </span>
                    </td>
