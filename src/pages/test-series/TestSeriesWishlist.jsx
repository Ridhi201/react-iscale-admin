...
export default function TestSeriesWishlist() {
  ...
  const fetchWishlists = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${BASE_URL}/myadmin/user-wishlist/test-pack/admin/all`, {
        params: {
          page: currentPage,
  ...
  const fetchPackages = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await axios.get(`${BASE_URL}/myadmin/test-package/dropdown`, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const handleView = async (id) => {
    try {
      const token = localStorage.getItem('token')
      const response = await axios.get(`${BASE_URL}/myadmin/user-wishlist/test-pack/admin/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data.status) {
  ...
  return (
    <div className="h-full animate-fade-in-up">
      ...
      <div className="bg-[#144f36] rounded-t-2xl p-5 flex justify-between items-center shadow-md relative overflow-hidden group mb-5">
        <h2 className="text-white font-bold tracking-tight text-xl relative z-10">Test Wishlist List</h2>
      </div>
      ...
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors p-5 mb-5">
        <div className="flex flex-wrap items-end gap-5 w-full">
          ...
          {/* Package */}
          <div className="flex-1 min-w-[200px]">
            <label className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">Test Package</label>
            <select 
              value={packageId}
            ...
            >
              ...
              {packages.map(p => (
                <option key={p._id} value={p._id}>{p.title || p.m_package_title}</option>
              ))}
            </select>
          </div>
          {/* Search */}
          ...
          <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
            <thead className="bg-slate-50 dark:bg-[#13111c] text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-gray-800">
              <tr>
                ...
                <th className="px-3 py-3 font-semibold text-xs border-r border-slate-200 dark:border-gray-800/50 align-top">Package Name</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-2xl relative shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-4">
              ...
              <div>
                <p className="font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider mb-1">Package Name</p>
                <p className="text-gray-800 dark:text-gray-200">{selectedWishlist.package_id?.m_package_title || selectedWishlist.package_id?.title}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider mb-1">Package Price</p>
                <p className="text-gray-800 dark:text-gray-200">₹{selectedWishlist.package_id?.m_package_price || 0}</p>
              </div>
              <div>
