...
import { allTestSeriesWishlistData } from '../../utils/mockData'

const TOTAL_ENTRIES = allTestSeriesWishlistData.length
...
export default function TestSeriesWishlist() {
  ...
  const currentData = allTestSeriesWishlistData.slice(startIndex, endIndex)
  ...
  return (
    <div className="h-full animate-fade-in-up">
      {/* Title Card */}
        ...
        <h2 className="text-indigo-900 dark:text-indigo-300 font-bold tracking-tight text-xl font-medium">Test Wishlist List</h2>
      </div>
          ...
          <table className="w-full text-left text-sm text-slate-800 dark:text-slate-200">
            ...
            <tbody>
              {currentData.map((row) => (
                <tr 
                  key={row.id} 
                  className={`border-b border-slate-200 dark:border-gray-800/50 ${row.sno === 10 ? 'bg-[#f6f6ff]' : 'bg-[#f6f6ff] dark:bg-[#111827] hover:bg-slate-50 dark:bg-[#1f1b2e]/50 dark:hover:bg-[#1f2937] transition-colors'}`}
                >
