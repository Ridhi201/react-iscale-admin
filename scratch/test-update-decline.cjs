...
async function run() {
  ...
  // Try updating to declined (3)
  try {
    const response = await axios.put(`${BASE_URL}/myadmin/certificate/update-status/${enrollment_id}`, {
      status: 3,
      certificate_status: 3,
      declined_reason: "Test Decline"
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    console.log("Decline Update Response:", response.data);
    ...
    console.log("Record details after decline update:", JSON.stringify(record, null, 2));
