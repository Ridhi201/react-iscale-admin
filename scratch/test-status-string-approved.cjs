...
async function run() {
  ...
  // Test 1: JSON with status: "approved"
  await resetStatus(token);
  try {
    const res = await axios.put(`${BASE_URL}/myadmin/certificate/update-status/${enrollment_id}`, {
      status: "approved",
      certificate_no: "TEST-APPROVED-STR-1",
      certificate_pdf: "https://example.com/cert.pdf"
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    console.log("Test 1 (JSON approved) response:", res.data);
  } catch (err) {
    console.log("Test 1 (JSON approved) failed:", err.response ? err.response.data : err.message);
  }
  let status = await checkStatus(token);
  console.log("Test 1 DB status:", status ? status.certificate_status : 'unknown');

  // Test 2: Multipart with status: "approved" and a file
  await resetStatus(token);
  const FormData = require('form-data');
  const form = new FormData();
  form.append('status', 'approved');
  form.append('certificate_no', 'TEST-APPROVED-STR-2');
  ...
  try {
    const res = await axios.put(`${BASE_URL}/myadmin/certificate/update-status/${enrollment_id}`, form, {
      headers: { 
        Authorization: `Bearer ${token}`,
        ...form.getHeaders()
      }
    });
    console.log("Test 2 (Multipart approved) response:", res.data);
  } catch (err) {
    console.log("Test 2 (Multipart approved) failed:", err.response ? err.response.data : err.message);
  }
